...
public:
    ...
    void build(const string& str) {
        ...
        for (auto p : freqMap) {
            pq.push(new BinNode(p.first, p.second));
        }
        while (pq.size() > 1) {
            BinNode* left = pq.top();
            pq.pop();
...
private:
    ...
    void buildHuffCode(BinNode* node, string code) {
        if (!node) {
            return;
        }
        if (!node->left && !node->right) {
